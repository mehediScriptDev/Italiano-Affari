          ...
          {notifications.length > 0 ? (
            notifications.map((notification) => (
              <div key={notification.id} className={`notification-item ${notification.is_read ? "read" : "unread"}`}>
                  ...
                  <p style={{ textDecoration: "underline", cursor: "pointer" }} hidden={notification.is_read} className="fw-medium mt-2" onClick={() => handleNotificationRead(notification.id)}>Visualizza</p>
                </div>
