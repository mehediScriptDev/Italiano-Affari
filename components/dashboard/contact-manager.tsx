  ...
  return (
    <>
      <Box className="flex justify-center px-2" sx={{ backgroundColor: "#f6f8fb" }}>
          <Box sx={{ maxWidth: "1200px" }} className="mt-5 w-full flex flex-col justify-center px-4 p-0">
            ...
            <Box>
              ...
              <Button color="secondary" variant="contained" onClick={() => { setOpenModal(true); setEditingContact(null); setFormData({ name: "", email: "" }); }} disabled={loading}>Aggiungi Contatto</Button>
            </Box>
          </Box>
      ...
      <Dialog open={openModal} fullWidth maxWidth="sm" onClose={() => setOpenModal(false)}>
        <DialogTitle textAlign="center">{editingContact ? "Modifica Contatto" : "Aggiungi Contatto"}</DialogTitle>
        <form onSubmit={handleSubmit}>
          <DialogContent>
            <Grid container spacing={2}>
              <Grid size={12}><TextField required fullWidth margin="dense" label="Nome" name="name" value={formData.name} onChange={(e) => setFormData((p) => ({ ...p, name: e.target.value }))} /></Grid>
